...
sub _normalize_ecs {
  ...
  my ($service, $task_def) = @$ecs{'service', 'taskDefinition'};
...
sub _normalize_elb_load_balancers {
  ...
  # we are defaulting to one LB per service at this time
  my ($lb, $lbs) = @$elb{qw(loadBalancer loadBalancers)};
  confess "loadBalancers not supported in <2.x" if defined $lbs;
  confess "Missing loadBalancer configuration in elb section" unless defined $lb;

  # rename loadBalancer to loadBalancers and put in array
  $elb->{loadBalancers} = [$lb];
  delete $elb->{loadBalancer};
...
sub _normalize_elb_load_balancer {
  ...
  my ($spec_listener, $spec_listeners) = @$lb{qw(listener listeners)};
  confess "loadBalancer.listeners not supported in <2.x" if defined $spec_listeners;
  my $temp_listener = delete %$lb{listener};
  $lb->{listeners} = [$temp_listener];
...
sub _normalize_elb_load_balancer_listener {
  ...
  $self->_normalize_elb_load_balancer_listener_ruleset($listener, $lb);
}
...
sub _normalize_elb_load_balancer_listener_ruleset {
  ...
  confess "listener.rules is not an object in $lb->{type} load balancer $lb->{name}"
    unless 'HASH' eq ref $ruleset;
  
  for my $role (keys %$ruleset) {
    my $rules = $ruleset->{$role};

    confess "listener ruleset for role $role is not an array in $lb->{type} load balancer $lb->{name}"
      unless 'ARRAY' eq ref $rules;

    $self->_normalize_elb_load_balancer_listener_rules($role, $rules, $lb);
  }
}

sub _normalize_elb_load_balancer_listener_rules {
  my ($self, $role, $rules, $lb) = @_;

  my $i = 0;

  for my $rule (@$rules) {
    $self->_normalize_elb_load_balancer_listener_rule($rules, $role, $i, $lb);
    $i++;
  }
}

sub _normalize_elb_load_balancer_listener_rule {
  my ($self, $rules, $role, $index, $lb) = @_;

  my $rule = $rules->[$index];
  my $role_err = $role ? "for role $role " : "";

  confess "listener rule at index $index ${role_err}is not an object ".
  ...
  confess "Invalid placement value '$rule->{placement}' for listener rule at index $index ".
          "${role_err}in $lb->{type} load balancer $lb->{name}"
    unless $rule->{placement} =~ /^(?:start|end)$/;
  
  # TODO Conditions validation

  $self->_validate_elb_load_balancer_listener_rule_action($rule, $role, $index, $lb);
}

sub _validate_elb_load_balancer_listener_rule_action {
  my ($self, $rule, $role, $index, $lb) = @_;
  ...
  confess "Invalid action for listener rule at index $index for role $role: " .
...
sub _normalize_elb_target_group_protocol {
  ...
  confess "Invalid target group protocol: should be HTTP or HTTPS, got $tg->{protocol} in ELB target group $tg->{name}"
...
sub _normalize_elb_target_group_health_check_protocol {
  ...
  confess "Invalid health check protocol in ELB target group $tg->{name}: " .
        "should be HTTP or HTTPS, got $hc->{protocol}"
...
sub _normalize_tests {
  ...
  confess "tests is not an object" unless 'HASH' eq ref $tests;

  $self->_normalize_test_suite($_, $tests->{$_}) for keys %$tests;
}
...
sub elb { shift->{spec}->{elb} }
sub ecs { shift->{spec}->{ecs} }
sub verification { shift->{spec}->{verification} }
sub tests { shift->{spec}->{tests} }
