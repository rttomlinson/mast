...
sub _normalize_route53 {
  ...
  lambda_confess "route53 is not an array"
...
sub _normalize_record {
  ...
  lambda_confess "Route53 DNS domain name is required in route53[$index], got '$domain'"
    unless $domain;
  
  lambda_confess "Route53 DNS record name is required in route53[$index], got '$name'"
    unless $name;
  ...
  if ('HASH' eq ref $value) {
    ...
    if ($aliasType =~ /^applicationloadbalancer$/i) {
      ...
      lambda_confess "Target alias for record $name in Route53 domain $domain should include ALB name"
...
sub _validate_top_keys {
  ...
  lambda_confess "AWS region is not defined"
...
sub _normalize_ecs {
  ...
  lambda_confess "ecs is not an object" unless 'HASH' eq ref $ecs;
  ...
  if(defined $ecs_tasks) {
    ...
    lambda_confess "ecs.tasks is not an object"
...
sub _normalize_ecs_task_definition {
  ...
  lambda_confess "taskDefinition is not an object" unless 'HASH' eq ref $task_def;

  my $container_defs = $task_def->{containerDefinitions};

  lambda_confess "containerDefinitions is not an array in ECS taskDefinition"
...
sub _normalize_container {
  ...
  lambda_confess "container configuration is not an object at containerDefinitions index $index"
  ...
  lambda_confess "Container name is required at containerDefinitions index $index"
    unless $container_name;
  lambda_confess "Container image is required in container definition $container_name"
  ...
  if (defined $env) {
    lambda_confess "environment is not an array in container definition $container_name"
  ...
  if (defined (my $secrets = $container->{secrets})) {
    lambda_confess "secrets not an array in container definition $container_name"
  ...
  if (defined (my $ports = $container->{portMappings})) {
    lambda_confess "portMappings is not an array in container definition $container_name"
      unless 'ARRAY' eq ref $ports;

    my $i = 0;    

    for my $pm (@$ports) {
      lambda_confess "portMappings entry at index $i is not an object in container definition $container_name"
  ...
  if (defined (my $logConfig = $container->{logConfiguration})) {
    lambda_confess "logConfiguration is not an object in container definition $container_name"
      unless 'HASH' eq ref $logConfig;

    if (defined (my $options = $logConfig->{options})) {
      lambda_confess "logConfiguration.options is not an object in container definition $container_name"
...
sub _check_env_var {
  ...
  lambda_confess q|Expected an object with "name" and "value" keys | .
...
sub _check_secret {
  ...
  lambda_confess q|Expected an object with "name" and "valueFrom" keys | .
  ...
  lambda_confess qq|Secret variable name "$secret->{name}" collides with environment | .
...
sub _normalize_ecs_service {
  ...
  lambda_confess "ecs.service is not an object" unless 'HASH' eq ref $service;

  my $service_name = $service->{name};

  my $network = $service->{networkConfiguration};

  lambda_confess "networkConfiguration is not an object in ECS service $service_name"
...
sub _normalize_ecs_service_load_balancers {
  ...
  lambda_confess "loadBalancers is not an array in ECS service $service->{name}"
...
sub _normalize_ecs_service_lb {
  ...
  lambda_confess "loadBalancers entry at index $index is not an object " .
          "in ECS service $service->{name}"
    unless 'HASH' eq ref $lb;

  my ($container_name, $container_port) = @$lb{'containerName', 'containerPort'};

  lambda_confess "loadBalancers entry at index $index is expected to have " .
  ...
  lambda_confess "Cannot find matching ELB target group configuration for container name $container_name"
    unless $found;
}
...
sub _normalize_ecs_scaling_policies_and_target {
  ...
  lambda_confess "scalingPolicy is not an object in ECS service $service->{name}"
  ...
  lambda_confess "scalableTarget is not an object in ECS service $service->{name}"
  ...
  lambda_confess "Scalable Target ResourceId is expected to match the pattern " .
          "'service/\$cluster_name/\$service_name' in ECS service $service->{name}"
    unless $scalable_target_resource_id =~ /^$expected_resource_id$/;

  lambda_confess "Scaling Policy ResourceId is expected to match the pattern " .
          "'service/\$cluster_name/\$service_name' in ECS service $service->{name}"
...
sub _validate_ecs_scaling_policy {
  ...
  if ($predefined_metric_type eq 'ALBRequestCountPerTarget') {
    ...
    lambda_confess "Scaling policy for ECS service $service->{name} is configured for ".
...
sub _normalize_elb {
  ...
  lambda_confess "elb is not an object" unless 'HASH' eq ref $elb;
...
sub _normalize_elb_load_balancers {
  ...
  lambda_confess "Missing elb.loadBalancers configuration in elb section" unless defined $lbs;
  lambda_confess "elb.loadBalancers is not an array" unless 'ARRAY' eq ref $lbs;
...
sub _normalize_elb_load_balancer {
  ...
  lambda_confess "loadBalancer is not an object" unless 'HASH' eq ref $lb;
  lambda_confess "elb.loadBalancer value requires a name keyword" unless exists $lb->{name};
  lambda_confess "elb.loadBalancer value requires a type keyword" unless exists $lb->{type};
  ...
  lambda_confess "elb.listeners is not an array in $lb->{type} load balancer $lb->{name}"
...
sub _validate_elb_load_balancers {
  ...
  lambda_confess "At least one repeat load balancer and listener port found. List: @all_matches. You can combine rules."
    ...
    for my $listener_spec (@$listener_specs) {
      if ($_->{type} eq 'network') {
        ...
        if(defined $target_group_name_to_elb_name{"$target_group_name"}){
          lambda_confess "Same target group cannot be on multiple different elbs" unless $target_group_name_to_elb_name{"$target_group_name"} eq $lb_name;
        } else {
...
sub _validate_elb_load_balancer {
  ...
  lambda_confess "elb.loadBalancer[$index] is not an object" unless 'HASH' eq ref $lb;
  lambda_confess "elb.loadBalancer[$index] value requires a name keyword" unless exists $lb->{name};
  lambda_confess "elb.loadBalancer[$index] value requires a type keyword" unless exists $lb->{type};
}

sub _validate_aws_elb_name {
  my ($self, $name) = @_;

  lambda_confess "Invalid load balancer name: null" unless defined $name;

  lambda_confess "Load balancer name $name is not scalar. This means that you probably did not provide an appropriate value for the environment you provided"
    if ref $name;
  
  lambda_confess qq|ELB load balancer name length cannot exceed 32 characters, got "$name"|
    if length $name > 32;
  
  lambda_confess qq|Invalid ELB load balancer name, only alphanumerics and hyphens are permitted: got "$name"|
   unless $name =~ /^[-a-zA-Z0-9]+$/;
  
  lambda_confess qq|Invalid ELB load balancer name "$name", cannot start or end with a hyphen|
    if $name =~ /^-|-$/;
}
...
sub _validate_aws_elb_type {
  ...
  lambda_confess "Unsupported type. '$lb->{type}' in load balancer $lb->{name}: " .
...
sub _normalize_elb_load_balancer_listener_protocol {
  ...
  if ($lb->{type} eq 'network') {
    lambda_confess "Invalid listener protocol in $lb->{type} load balancer $lb->{name}: " .
...
sub _normalize_elb_load_balancer_listener_port {
  ...
  lambda_confess "Invalid listener port in $lb->{type} load balancer $lb->{name}: expected integer > 0, got $listener->{port}"
...
sub _normalize_elb_load_balancer_listener_ruleset {
  ...
  lambda_confess "listener.rules is not an array in $lb->{type} load balancer $lb->{name}"
...
sub _normalize_elb_load_balancer_listener_rule {
  ...
  lambda_confess "listener rule at index $index is not an object ".
          "in $lb->{type} load balancer $lb->{name}"
    unless 'HASH' eq ref $rule;

  $rule->{placement} = lc $self->stringify($rule->{placement});

  lambda_confess "Invalid placement value '$rule->{placement}' for listener rule at index $index ".
          "in $lb->{type} load balancer $lb->{name}"
...
sub _validate_elb_load_balancer_listener_rule_action {
  ...
  lambda_confess "Action types other than 'forward' are not supported at this time"
  ...
  lambda_confess "Invalid action for listener rule at index $index: " .
...
sub _normalize_elb_target_groups {
  ...
  lambda_confess "Missing target group configuration in elb section" unless defined $tgs;
  lambda_confess "elb.targetGroups is not an array" unless 'ARRAY' eq ref $tgs;
...
sub _normalize_elb_target_group {
  ...
  lambda_confess "elb.targetGroups entry at index $index is not an object"
...
sub _validate_elb_target_group_name {
  ...
  lambda_confess "Target group name is required at elb.targetGroups index $index"
    unless length $name > 0;

  lambda_confess qq|Target group name length cannot exceed 32 characters, got "$name" at elb.targetGroups index $index|
    if length $name > 32;    

  lambda_confess qq|Invalid target group name "$name", only alphanumerics and hyphens are permitted at elb.targetGroups index $index|
    unless $name =~ /^[-a-zA-Z0-9]+$/;

  lambda_confess qq|Invalid target group name "$name", cannot start or end with a hyphen at elb.targetGroups index $index|
    if $name =~ /^-|-$/;
...
sub _normalize_elb_target_group_protocol {
  ...
  if ($lb->{type} eq 'network') {
    lambda_confess "Invalid target group protocol in $lb->{type} load balancer $lb->{name}: " .
...
sub _normalize_elb_target_group_health_check {
  ...
  lambda_confess "healthCheck is not an object in ELB target group $tg->{name}"
...
sub _normalize_elb_target_group_health_check_protocol {
  ...
  if ($tg->{protocol} =~ /^TCP$/) {
    lambda_confess "Invalid health check protocol in ELB target group $tg->{name}: " .
...
sub _normalize_elb_target_group_health_check_matcher {
  ...
  lambda_confess "healthCheck.matcher is not an object in ELB target group $tg->{name}"
...
sub _normalize_verification {
  ...
  lambda_confess "verification is not an object"
  ...
  if (defined (my $request = $verification->{request})) {
    lambda_confess "verification.request is not an object"
      unless 'HASH' eq ref $request;
    
    lambda_confess "verification.request.headers is not an array"
    ...
    for my $header (@{ $request->{headers} || [] }) {
      if ('HASH' eq ref $header) {
        ...
        lambda_confess "Array of headers is not expected at verification.request.headers entry index $i";
      }
      else {
        push @headers, $header;
      }
  ...
  if (defined (my $response = $verification->{response})) {
    lambda_confess "verification.response is not an object"
      unless 'HASH' eq ref $response;
    
    $response->{status} = [$response->{status}]
      if defined($response->{status}) and 'ARRAY' ne ref $response->{status};
    
    lambda_confess "verification.response.body should be a string"
...
sub _normalize_tests {
  ...
  lambda_confess "tests section is not supported, use ecs.tasks section instead"
...
#   lambda_confess "Test suite $test_name: types other than ecsTask are not supported at the time"
...
sub _normalize_ecs_task {
  ...
  lambda_confess "ECS task $task_name should be an object"
    unless 'HASH' eq ref $task;

  lambda_confess "ECS task $task_name: Cluster name is required"
    unless defined $task->{cluster};

  my $task_def = $task->{taskDefinition};

  lambda_confess "ECS task $task_name: task family is required"
  ...
  lambda_confess "containerDefinitions should be an array in ECS task $task_name"
    unless 'ARRAY' eq ref $container_defs;

  for my $container (@$container_defs) {
    my $log_config = $container->{logConfiguration};

    lambda_confess "ECS task $task_name: Log drivers other than awslogs are not supported at this time"
      unless $log_config->{logDriver} eq 'awslogs';
    
    my $options = $log_config->{options};

    lambda_confess "ECS task $task_name: Log configuration options should include 'awslogs-stream-prefix' and 'awslogs-group' properties"
      if not $options or not $options->{'awslogs-stream-prefix'}
          or not $options->{'awslogs-group'};
  }

  if ($task_def->{networkMode} eq 'awsvpc') {
    lambda_confess "ECS task $task_name: Task definition set to awsvpc but networkConfiguration section is not found"
